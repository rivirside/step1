// Antimicrobials - Drugs
// Auto-generated from modularize-data.js

const drugs = [

    {
        "id": "penicillin-g",
        "name": "Penicillin G",
        "pharmacologicClass": "penicillins",
        "therapeuticClass": "cell-wall-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Bind to penicillin-binding proteins (PBPs) to inhibit peptidoglycan synthesis.",
        "features": "Narrow spectrum, IV/IM only.",
        "clinicalChoice": "Syphilis, strep pharyngitis.",
        "indications": [
            "Syphilis",
            "Strep Pharyngitis"
        ],
        "contraindications": [
            "Penicillin Allergy"
        ],
        "sideEffects": [
            "Hypersensitivity reactions",
            "Anaphylaxis",
            "Interstitial nephritis"
        ],
        "interactions": "Probenecid increases levels, tetracyclines may interfere.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "amoxicillin",
        "name": "Amoxicillin",
        "pharmacologicClass": "penicillins",
        "therapeuticClass": "cell-wall-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Bind to penicillin-binding proteins (PBPs) to inhibit peptidoglycan synthesis.",
        "features": "Oral bioavailability, broader spectrum.",
        "clinicalChoice": "Otitis media, sinusitis.",
        "indications": [
            "Otitis Media",
            "Sinusitis"
        ],
        "contraindications": [
            "Penicillin Allergy"
        ],
        "sideEffects": [
            "Hypersensitivity reactions",
            "Anaphylaxis",
            "Interstitial nephritis"
        ],
        "interactions": "Probenecid increases levels, tetracyclines may interfere.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "nafcillin",
        "name": "Nafcillin",
        "pharmacologicClass": "penicillins",
        "therapeuticClass": "cell-wall-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Bind to penicillin-binding proteins (PBPs) to inhibit peptidoglycan synthesis.",
        "features": "Penicillinase-resistant.",
        "clinicalChoice": "MSSA infections.",
        "indications": [
            "MSSA Infections"
        ],
        "contraindications": [
            "Penicillin Allergy"
        ],
        "sideEffects": [
            "Hypersensitivity reactions",
            "Anaphylaxis",
            "Interstitial nephritis"
        ],
        "interactions": "Probenecid increases levels, tetracyclines may interfere.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "cefazolin-1st-gen",
        "name": "Cefazolin (1st gen)",
        "pharmacologicClass": "cephalosporins",
        "therapeuticClass": "cell-wall-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Similar to penicillins; inhibit cell wall synthesis. Organized by generations.",
        "features": "Excellent gram-positive coverage.",
        "clinicalChoice": "Surgical prophylaxis.",
        "indications": [
            "Surgical Prophylaxis"
        ],
        "contraindications": [
            "Cephalosporin Allergy"
        ],
        "sideEffects": [
            "Hypersensitivity",
            "Disulfiram-like reaction",
            "Vitamin K deficiency"
        ],
        "interactions": "Aminoglycosides increase nephrotoxicity.",
        "interactionDetails": "",
        "pageType": "drug"
    },
    {
        "id": "ceftriaxone-3rd-gen",
        "name": "Ceftriaxone (3rd gen)",
        "pharmacologicClass": "cephalosporins",
        "therapeuticClass": "cell-wall-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Similar to penicillins; inhibit cell wall synthesis. Organized by generations.",
        "features": "CNS penetration, broad spectrum.",
        "clinicalChoice": "Meningitis, gonorrhea.",
        "indications": [
            "Meningitis",
            "Gonorrhea"
        ],
        "contraindications": [
            "Cephalosporin Allergy"
        ],
        "sideEffects": [
            "Hypersensitivity",
            "Disulfiram-like reaction",
            "Vitamin K deficiency"
        ],
        "interactions": "Aminoglycosides increase nephrotoxicity.",
        "interactionDetails": "",
        "pageType": "drug"
    },
    {
        "id": "cefepime-4th-gen",
        "name": "Cefepime (4th gen)",
        "pharmacologicClass": "cephalosporins",
        "therapeuticClass": "cell-wall-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Similar to penicillins; inhibit cell wall synthesis. Organized by generations.",
        "features": "Anti-pseudomonal activity.",
        "clinicalChoice": "Hospital-acquired pneumonia.",
        "indications": [
            "Hospital-Acquired Pneumonia"
        ],
        "contraindications": [
            "Cephalosporin Allergy"
        ],
        "sideEffects": [
            "Hypersensitivity",
            "Disulfiram-like reaction",
            "Vitamin K deficiency"
        ],
        "interactions": "Aminoglycosides increase nephrotoxicity.",
        "interactionDetails": "",
        "pageType": "drug"
    },
    {
        "id": "imipenem-cilastatin",
        "name": "Imipenem/Cilastatin",
        "pharmacologicClass": "carbapenems",
        "therapeuticClass": "cell-wall-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Beta-lactams resistant to most beta-lactamases, broad-spectrum activity.",
        "features": "Cilastatin prevents renal dehydropeptidase degradation.",
        "clinicalChoice": "Severe polymicrobial infections.",
        "indications": [
            "Severe Nosocomial Infections"
        ],
        "contraindications": [
            "History of Seizures"
        ],
        "sideEffects": [
            "Seizures (imipenem)",
            "GI upset",
            "Rash"
        ],
        "interactions": "Valproate levels decreased, raising seizure risk.",
        "interactionDetails": "",
        "pageType": "drug"
    },
    {
        "id": "meropenem",
        "name": "Meropenem",
        "pharmacologicClass": "carbapenems",
        "therapeuticClass": "cell-wall-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Beta-lactams resistant to most beta-lactamases, broad-spectrum activity.",
        "features": "Lower seizure risk, stable to dehydropeptidase.",
        "clinicalChoice": "Resistant gram-negative infections.",
        "indications": [
            "Resistant Gram-negative Infections"
        ],
        "contraindications": [
            "Hypersensitivity"
        ],
        "sideEffects": [
            "Seizures (imipenem)",
            "GI upset",
            "Rash"
        ],
        "interactions": "Valproate levels decreased, raising seizure risk.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "aztreonam",
        "name": "Aztreonam",
        "pharmacologicClass": "monobactams",
        "therapeuticClass": "cell-wall-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Monocyclic beta-lactam active against gram-negative rods.",
        "features": "Safe in penicillin allergy, lacks gram-positive/anaerobe activity.",
        "clinicalChoice": "Gram-negative infections in beta-lactam allergy.",
        "indications": [
            "Gram-negative Infections"
        ],
        "contraindications": [
            "Hypersensitivity"
        ],
        "sideEffects": [
            "Rash",
            "Elevated liver enzymes"
        ],
        "interactions": "Minimal cross-reactivity with other beta-lactams.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "vancomycin",
        "name": "Vancomycin",
        "pharmacologicClass": "glycopeptides",
        "therapeuticClass": "cell-wall-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Bind D-Ala-D-Ala termini to inhibit peptidoglycan cross-linking.",
        "features": "IV for MRSA, oral for C. difficile.",
        "clinicalChoice": "Serious gram-positive infections.",
        "indications": [
            "MRSA Infections",
            "C. difficile Colitis"
        ],
        "contraindications": [
            "Hypersensitivity"
        ],
        "sideEffects": [
            "Red man syndrome",
            "Nephrotoxicity",
            "Ototoxicity"
        ],
        "interactions": "Other nephrotoxic drugs increase toxicity.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "daptomycin",
        "name": "Daptomycin",
        "pharmacologicClass": "lipopeptides",
        "therapeuticClass": "cell-wall-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Insert into gram-positive cell membranes causing depolarization and rapid cell death.",
        "features": "Bactericidal against MRSA and VRE, inactivated by lung surfactant.",
        "clinicalChoice": "Right-sided endocarditis, complicated skin infections.",
        "indications": [
            "MRSA Infections",
            "VRE Infections"
        ],
        "contraindications": [
            "Pneumonia"
        ],
        "sideEffects": [
            "Myopathy",
            "Eosinophilic pneumonia"
        ],
        "interactions": "Statins increase risk of myopathy.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "gentamicin",
        "name": "Gentamicin",
        "pharmacologicClass": "aminoglycosides",
        "therapeuticClass": "protein-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Inhibit 30S ribosomal subunit, causing misreading of mRNA.",
        "features": "Broad-spectrum, once-daily dosing.",
        "clinicalChoice": "Gram-negative sepsis.",
        "indications": [
            "Gram-negative Infections",
            "Endocarditis"
        ],
        "contraindications": [
            "Pregnancy",
            "Myasthenia Gravis"
        ],
        "sideEffects": [
            "Nephrotoxicity",
            "Ototoxicity",
            "Neuromuscular blockade"
        ],
        "interactions": "Loop diuretics increase ototoxicity.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "tobramycin",
        "name": "Tobramycin",
        "pharmacologicClass": "aminoglycosides",
        "therapeuticClass": "protein-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Inhibit 30S ribosomal subunit, causing misreading of mRNA.",
        "features": "Anti-pseudomonal activity.",
        "clinicalChoice": "Pseudomonas infections.",
        "indications": [
            "Pseudomonas Infections"
        ],
        "contraindications": [
            "Pregnancy",
            "Renal Impairment"
        ],
        "sideEffects": [
            "Nephrotoxicity",
            "Ototoxicity",
            "Neuromuscular blockade"
        ],
        "interactions": "Loop diuretics increase ototoxicity.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "amikacin",
        "name": "Amikacin",
        "pharmacologicClass": "aminoglycosides",
        "therapeuticClass": "protein-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Inhibit 30S ribosomal subunit, causing misreading of mRNA.",
        "features": "Resistant to most aminoglycoside-modifying enzymes.",
        "clinicalChoice": "MDR gram-negative infections.",
        "indications": [
            "MDR Gram-negative Infections"
        ],
        "contraindications": [
            "Pregnancy",
            "Hearing Loss"
        ],
        "sideEffects": [
            "Nephrotoxicity",
            "Ototoxicity",
            "Neuromuscular blockade"
        ],
        "interactions": "Loop diuretics increase ototoxicity.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "streptomycin",
        "name": "Streptomycin",
        "pharmacologicClass": "aminoglycosides",
        "therapeuticClass": "protein-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Inhibit 30S ribosomal subunit, causing misreading of mRNA.",
        "features": "Oldest agent, significant ototoxicity.",
        "clinicalChoice": "Second-line tuberculosis, plague.",
        "indications": [
            "Tuberculosis",
            "Yersinia pestis Infection"
        ],
        "contraindications": [
            "Pregnancy",
            "Myasthenia Gravis"
        ],
        "sideEffects": [
            "Nephrotoxicity",
            "Ototoxicity",
            "Neuromuscular blockade"
        ],
        "interactions": "Loop diuretics increase ototoxicity.",
        "interactionDetails": "",
        "pageType": "drug"
    },
    {
        "id": "azithromycin",
        "name": "Azithromycin",
        "pharmacologicClass": "macrolides",
        "therapeuticClass": "protein-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Inhibit 50S ribosomal subunit, block peptide elongation.",
        "features": "Long half-life, tissue penetration.",
        "clinicalChoice": "Atypical pneumonia, chlamydia.",
        "indications": [
            "Atypical Pneumonia",
            "Chlamydia"
        ],
        "contraindications": [
            "QT Prolongation"
        ],
        "sideEffects": [
            "GI upset",
            "QT prolongation",
            "Hepatotoxicity"
        ],
        "interactions": "CYP3A4 inhibition, increases levels of many drugs.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "clarithromycin",
        "name": "Clarithromycin",
        "pharmacologicClass": "macrolides",
        "therapeuticClass": "protein-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Inhibit 50S ribosomal subunit, block peptide elongation.",
        "features": "H. pylori activity.",
        "clinicalChoice": "H. pylori eradication.",
        "indications": [
            "H. pylori Infection"
        ],
        "contraindications": [
            "Liver Disease"
        ],
        "sideEffects": [
            "GI upset",
            "QT prolongation",
            "Hepatotoxicity"
        ],
        "interactions": "CYP3A4 inhibition, increases levels of many drugs.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "erythromycin",
        "name": "Erythromycin",
        "pharmacologicClass": "macrolides",
        "therapeuticClass": "protein-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Inhibit 50S ribosomal subunit, block peptide elongation.",
        "features": "Motilin receptor agonist.",
        "clinicalChoice": "Gastroparesis, penicillin allergy.",
        "indications": [
            "Gastroparesis",
            "Strep Infections"
        ],
        "contraindications": [
            "QT Prolongation"
        ],
        "sideEffects": [
            "GI upset",
            "QT prolongation",
            "Hepatotoxicity"
        ],
        "interactions": "CYP3A4 inhibition, increases levels of many drugs.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "doxycycline",
        "name": "Doxycycline",
        "pharmacologicClass": "oral-antibiotics",
        "therapeuticClass": "protein-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Inhibit 30S ribosomal subunit, prevent tRNA binding.",
        "features": "Lipophilic, good tissue penetration. | Tetracycline, anti-inflammatory properties.",
        "clinicalChoice": "Lyme disease, rickettsial infections.",
        "indications": [
            "Acne Vulgaris",
            "Lyme Disease",
            "Rocky Mountain Spotted Fever",
            "Rosacea"
        ],
        "contraindications": [
            "Pregnancy",
            "Children <8 years"
        ],
        "sideEffects": [
            "Tooth discoloration",
            "Photosensitivity",
            "Esophagitis"
        ],
        "interactions": "Divalent cations reduce absorption.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug",
        "pharmacologicClasses": [
            "oral-antibiotics",
            "tetracyclines"
        ]
    },
    {
        "id": "minocycline",
        "name": "Minocycline",
        "pharmacologicClass": "oral-antibiotics",
        "therapeuticClass": "protein-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Inhibit 30S ribosomal subunit, prevent tRNA binding.",
        "features": "CNS penetration, anti-inflammatory. | Lipophilic tetracycline, CNS penetration.",
        "clinicalChoice": "CNS infections, acne.",
        "indications": [
            "Acne",
            "Meningococcal Carrier State",
            "Severe Acne"
        ],
        "contraindications": [
            "Pregnancy",
            "Lupus"
        ],
        "sideEffects": [
            "Tooth discoloration",
            "Photosensitivity",
            "Esophagitis"
        ],
        "interactions": "Divalent cations reduce absorption.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug",
        "pharmacologicClasses": [
            "oral-antibiotics",
            "tetracyclines"
        ]
    },
    {
        "id": "tetracycline",
        "name": "Tetracycline",
        "pharmacologicClass": "tetracyclines",
        "therapeuticClass": "protein-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Inhibit 30S ribosomal subunit, prevent tRNA binding.",
        "features": "Original tetracycline.",
        "clinicalChoice": "H. pylori, acne.",
        "indications": [
            "H. pylori Infection",
            "Acne"
        ],
        "contraindications": [
            "Pregnancy",
            "Renal Impairment"
        ],
        "sideEffects": [
            "Tooth discoloration",
            "Photosensitivity",
            "Esophagitis"
        ],
        "interactions": "Divalent cations reduce absorption.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "chloramphenicol",
        "name": "Chloramphenicol",
        "pharmacologicClass": "chloramphenicol",
        "therapeuticClass": "protein-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Inhibits 50S ribosomal subunit, blocks peptidyl transferase.",
        "features": "Broad spectrum, CNS penetration.",
        "clinicalChoice": "Bacterial meningitis (when others contraindicated).",
        "indications": [
            "Bacterial Meningitis"
        ],
        "contraindications": [
            "Pregnancy",
            "G6PD Deficiency"
        ],
        "sideEffects": [
            "Bone marrow suppression",
            "Gray baby syndrome",
            "Aplastic anemia"
        ],
        "interactions": "Inhibits CYP enzymes.",
        "interactionDetails": "",
        "pageType": "drug"
    },
    {
        "id": "clindamycin",
        "name": "Clindamycin",
        "pharmacologicClass": "lincosamides",
        "therapeuticClass": "protein-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Bind 50S ribosomal subunit to inhibit peptide transfer.",
        "features": "Excellent anaerobic coverage above diaphragm.",
        "clinicalChoice": "Aspiration pneumonia, oral infections, toxin suppression.",
        "indications": [
            "Anaerobic Infections",
            "Streptococcal Toxic Shock"
        ],
        "contraindications": [
            "History of C. difficile Colitis"
        ],
        "sideEffects": [
            "C. difficile colitis",
            "Diarrhea",
            "Rash"
        ],
        "interactions": "Antagonistic when combined with macrolides.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "linezolid",
        "name": "Linezolid",
        "pharmacologicClass": "oxazolidinones",
        "therapeuticClass": "protein-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Prevent formation of the initiation complex on the 50S ribosomal subunit.",
        "features": "Oral option for resistant gram-positive organisms.",
        "clinicalChoice": "MRSA or VRE infections.",
        "indications": [
            "MRSA Infections",
            "VRE Infections"
        ],
        "contraindications": [
            "Uncontrolled Hypertension",
            "Serotonergic Therapy without adjustment"
        ],
        "sideEffects": [
            "Thrombocytopenia",
            "Peripheral neuropathy",
            "Serotonin syndrome"
        ],
        "interactions": "MAO inhibition potentiates serotonergic drugs.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "ciprofloxacin",
        "name": "Ciprofloxacin",
        "pharmacologicClass": "fluoroquinolones",
        "therapeuticClass": "dna-rna-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Inhibit DNA gyrase and topoisomerase IV.",
        "features": "Excellent gram-negative coverage.",
        "clinicalChoice": "UTI, gram-negative infections.",
        "indications": [
            "UTI",
            "Gram-negative Infections"
        ],
        "contraindications": [
            "Pregnancy",
            "Children"
        ],
        "sideEffects": [
            "Tendon rupture",
            "CNS effects",
            "QT prolongation"
        ],
        "interactions": "Divalent cations reduce absorption.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "levofloxacin",
        "name": "Levofloxacin",
        "pharmacologicClass": "fluoroquinolones",
        "therapeuticClass": "dna-rna-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Inhibit DNA gyrase and topoisomerase IV.",
        "features": "Respiratory quinolone, atypical coverage.",
        "clinicalChoice": "Community-acquired pneumonia.",
        "indications": [
            "Pneumonia",
            "Sinusitis"
        ],
        "contraindications": [
            "Pregnancy",
            "Tendon Disorders"
        ],
        "sideEffects": [
            "Tendon rupture",
            "CNS effects",
            "QT prolongation"
        ],
        "interactions": "Divalent cations reduce absorption.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "moxifloxacin",
        "name": "Moxifloxacin",
        "pharmacologicClass": "fluoroquinolones",
        "therapeuticClass": "dna-rna-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Inhibit DNA gyrase and topoisomerase IV.",
        "features": "Enhanced anaerobic and gram-positive coverage.",
        "clinicalChoice": "Complicated skin infections.",
        "indications": [
            "Skin and Soft Tissue Infections"
        ],
        "contraindications": [
            "Pregnancy",
            "QT Prolongation"
        ],
        "sideEffects": [
            "Tendon rupture",
            "CNS effects",
            "QT prolongation"
        ],
        "interactions": "Divalent cations reduce absorption.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "metronidazole",
        "name": "Metronidazole",
        "pharmacologicClass": "metronidazole",
        "therapeuticClass": "dna-rna-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "DNA damage via reduction to toxic metabolites in anaerobic conditions.",
        "features": "Excellent anaerobic coverage.",
        "clinicalChoice": "C. diff colitis, anaerobic infections.",
        "indications": [
            "C. diff Colitis",
            "Anaerobic Infections",
            "Trichomoniasis"
        ],
        "contraindications": [
            "Alcohol Use",
            "Pregnancy (1st trimester)"
        ],
        "sideEffects": [
            "Disulfiram-like reaction",
            "Peripheral neuropathy",
            "Metallic taste"
        ],
        "interactions": "Warfarin effects enhanced.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "sulfamethoxazole",
        "name": "Sulfamethoxazole",
        "pharmacologicClass": "sulfonamides",
        "therapeuticClass": "folate-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Inhibit dihydropteroate synthase, disrupt folate synthesis.",
        "features": "Usually combined with trimethoprim.",
        "clinicalChoice": "UTI, PCP prophylaxis.",
        "indications": [
            "UTI",
            "PCP Prophylaxis"
        ],
        "contraindications": [
            "Sulfa Allergy",
            "G6PD Deficiency"
        ],
        "sideEffects": [
            "Hypersensitivity",
            "Stevens-Johnson syndrome",
            "Kernicterus"
        ],
        "interactions": "Warfarin effects enhanced.",
        "interactionDetails": "",
        "pageType": "drug"
    },
    {
        "id": "trimethoprim-sulfamethoxazole",
        "name": "Trimethoprim-Sulfamethoxazole",
        "pharmacologicClass": "trimethoprim",
        "therapeuticClass": "folate-synthesis-inhibitors",
        "system": "antimicrobials",
        "mechanism": "Inhibits dihydrofolate reductase.",
        "features": "Synergistic combination. | Synergistic folate antagonism.",
        "clinicalChoice": "PCP treatment, MRSA.",
        "indications": [
            "MRSA Infections",
            "PCP Pneumonia",
            "PCP Treatment",
            "UTI"
        ],
        "contraindications": [
            "Sulfa Allergy",
            "Folate Deficiency"
        ],
        "sideEffects": [
            "Hyperkalemia",
            "Megaloblastic anemia",
            "Rash"
        ],
        "interactions": "ACE inhibitors increase hyperkalemia risk.",
        "interactionDetails": "",
        "pageType": "drug",
        "pharmacologicClasses": [
            "trimethoprim",
            "urinary-antiseptics"
        ]
    },
    {
        "id": "amphotericin-b",
        "name": "Amphotericin B",
        "pharmacologicClass": "polyenes",
        "therapeuticClass": "antifungals",
        "system": "antimicrobials",
        "mechanism": "Bind ergosterol in fungal cell membrane, create pores.",
        "features": "Broad-spectrum antifungal.",
        "clinicalChoice": "Systemic fungal infections.",
        "indications": [
            "Systemic Fungal Infections"
        ],
        "contraindications": [
            "Renal Impairment"
        ],
        "sideEffects": [
            "Nephrotoxicity",
            "Infusion reactions",
            "Electrolyte abnormalities"
        ],
        "interactions": "Nephrotoxic drugs increase risk.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "nystatin",
        "name": "Nystatin",
        "pharmacologicClass": "polyenes",
        "therapeuticClass": "antifungals",
        "system": "antimicrobials",
        "mechanism": "Bind ergosterol in fungal cell membrane, create pores.",
        "features": "Topical use only, not absorbed.",
        "clinicalChoice": "Oral/vaginal candidiasis.",
        "indications": [
            "Oral Candidiasis",
            "Vaginal Candidiasis"
        ],
        "contraindications": [
            "Hypersensitivity"
        ],
        "sideEffects": [
            "Nephrotoxicity",
            "Infusion reactions",
            "Electrolyte abnormalities"
        ],
        "interactions": "Nephrotoxic drugs increase risk.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "fluconazole",
        "name": "Fluconazole",
        "pharmacologicClass": "azoles",
        "therapeuticClass": "antifungals",
        "system": "antimicrobials",
        "mechanism": "Inhibit 14α-demethylase, disrupt ergosterol synthesis.",
        "features": "Excellent CNS penetration.",
        "clinicalChoice": "Candidiasis, cryptococcal meningitis.",
        "indications": [
            "Candidiasis",
            "Cryptococcal Meningitis"
        ],
        "contraindications": [
            "Liver Disease"
        ],
        "sideEffects": [
            "Hepatotoxicity",
            "Drug interactions",
            "QT prolongation"
        ],
        "interactions": "CYP3A4 inhibition affects many drugs.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "itraconazole",
        "name": "Itraconazole",
        "pharmacologicClass": "azoles",
        "therapeuticClass": "antifungals",
        "system": "antimicrobials",
        "mechanism": "Inhibit 14α-demethylase, disrupt ergosterol synthesis.",
        "features": "Broad spectrum, capsule requires acid.",
        "clinicalChoice": "Histoplasmosis, aspergillosis.",
        "indications": [
            "Histoplasmosis",
            "Aspergillosis"
        ],
        "contraindications": [
            "Heart Failure",
            "Liver Disease"
        ],
        "sideEffects": [
            "Hepatotoxicity",
            "Drug interactions",
            "QT prolongation"
        ],
        "interactions": "CYP3A4 inhibition affects many drugs.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "voriconazole",
        "name": "Voriconazole",
        "pharmacologicClass": "azoles",
        "therapeuticClass": "antifungals",
        "system": "antimicrobials",
        "mechanism": "Inhibit 14α-demethylase, disrupt ergosterol synthesis.",
        "features": "First-line for aspergillosis.",
        "clinicalChoice": "Invasive aspergillosis.",
        "indications": [
            "Invasive Aspergillosis"
        ],
        "contraindications": [
            "CYP2C19 Poor Metabolizers"
        ],
        "sideEffects": [
            "Hepatotoxicity",
            "Drug interactions",
            "QT prolongation"
        ],
        "interactions": "CYP3A4 inhibition affects many drugs.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "caspofungin",
        "name": "Caspofungin",
        "pharmacologicClass": "echinocandins",
        "therapeuticClass": "antifungals",
        "system": "antimicrobials",
        "mechanism": "Inhibit β-glucan synthesis in fungal cell wall.",
        "features": "First echinocandin, IV only.",
        "clinicalChoice": "Candidemia, aspergillosis.",
        "indications": [
            "Candidemia",
            "Invasive Aspergillosis"
        ],
        "contraindications": [
            "Hypersensitivity"
        ],
        "sideEffects": [
            "Infusion reactions",
            "Hepatotoxicity",
            "Histamine release"
        ],
        "interactions": "Few significant interactions.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "acyclovir",
        "name": "Acyclovir",
        "pharmacologicClass": "nucleoside-analogs",
        "therapeuticClass": "antivirals",
        "system": "antimicrobials",
        "mechanism": "Mimic nucleosides, terminate DNA/RNA synthesis.",
        "features": "HSV and VZV selective.",
        "clinicalChoice": "HSV encephalitis, VZV infections.",
        "indications": [
            "HSV Infections",
            "VZV Infections"
        ],
        "contraindications": [
            "Dehydration"
        ],
        "sideEffects": [
            "Bone marrow suppression",
            "Nephrotoxicity",
            "CNS effects"
        ],
        "interactions": "Probenecid increases levels.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "ganciclovir",
        "name": "Ganciclovir",
        "pharmacologicClass": "nucleoside-analogs",
        "therapeuticClass": "antivirals",
        "system": "antimicrobials",
        "mechanism": "Mimic nucleosides, terminate DNA/RNA synthesis.",
        "features": "CMV activity, more toxic.",
        "clinicalChoice": "CMV retinitis in AIDS.",
        "indications": [
            "CMV Infections"
        ],
        "contraindications": [
            "Neutropenia"
        ],
        "sideEffects": [
            "Bone marrow suppression",
            "Nephrotoxicity",
            "CNS effects"
        ],
        "interactions": "Probenecid increases levels.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "ribavirin",
        "name": "Ribavirin",
        "pharmacologicClass": "nucleoside-analogs",
        "therapeuticClass": "antivirals",
        "system": "antimicrobials",
        "mechanism": "Mimic nucleosides, terminate DNA/RNA synthesis.",
        "features": "Broad-spectrum antiviral.",
        "clinicalChoice": "RSV, hepatitis C (with interferon).",
        "indications": [
            "RSV",
            "Hepatitis C"
        ],
        "contraindications": [
            "Pregnancy",
            "Anemia"
        ],
        "sideEffects": [
            "Bone marrow suppression",
            "Nephrotoxicity",
            "CNS effects"
        ],
        "interactions": "Probenecid increases levels.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "oseltamivir",
        "name": "Oseltamivir",
        "pharmacologicClass": "neuraminidase-inhibitors",
        "therapeuticClass": "antivirals",
        "system": "antimicrobials",
        "mechanism": "Inhibit viral neuraminidase, prevent viral release.",
        "features": "Oral bioavailability.",
        "clinicalChoice": "Influenza treatment and prophylaxis.",
        "indications": [
            "Influenza"
        ],
        "contraindications": [
            "Hypersensitivity"
        ],
        "sideEffects": [
            "Nausea",
            "Vomiting",
            "Neuropsychiatric effects"
        ],
        "interactions": "Few significant interactions.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "zanamivir",
        "name": "Zanamivir",
        "pharmacologicClass": "neuraminidase-inhibitors",
        "therapeuticClass": "antivirals",
        "system": "antimicrobials",
        "mechanism": "Inhibit viral neuraminidase, prevent viral release.",
        "features": "Inhaled formulation.",
        "clinicalChoice": "Influenza (when oseltamivir resistant).",
        "indications": [
            "Influenza"
        ],
        "contraindications": [
            "Asthma",
            "COPD"
        ],
        "sideEffects": [
            "Nausea",
            "Vomiting",
            "Neuropsychiatric effects"
        ],
        "interactions": "Few significant interactions.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "chloroquine",
        "name": "Chloroquine",
        "pharmacologicClass": "quinoline-antimalarials",
        "therapeuticClass": "antimalarials",
        "system": "antimicrobials",
        "mechanism": "Interfere with heme detoxification in Plasmodium parasites.",
        "features": "Oral and parenteral, resistance widespread.",
        "clinicalChoice": "Malaria in chloroquine-sensitive areas.",
        "indications": [
            "Malaria",
            "Amebiasis"
        ],
        "contraindications": [
            "G6PD Deficiency",
            "Retinal Disease"
        ],
        "sideEffects": [
            "Cinchonism",
            "Hemolysis in G6PD deficiency",
            "Retinal toxicity"
        ],
        "interactions": "QT-prolonging drugs increase arrhythmia risk.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "quinine",
        "name": "Quinine",
        "pharmacologicClass": "quinoline-antimalarials",
        "therapeuticClass": "antimalarials",
        "system": "antimicrobials",
        "mechanism": "Interfere with heme detoxification in Plasmodium parasites.",
        "features": "IV for severe malaria, narrow therapeutic window.",
        "clinicalChoice": "Severe malaria, chloroquine-resistant P. falciparum.",
        "indications": [
            "Severe Malaria"
        ],
        "contraindications": [
            "G6PD Deficiency",
            "Optic Neuritis"
        ],
        "sideEffects": [
            "Cinchonism",
            "Hemolysis in G6PD deficiency",
            "Retinal toxicity"
        ],
        "interactions": "QT-prolonging drugs increase arrhythmia risk.",
        "interactionDetails": "",
        "pageType": "drug"
    },
    {
        "id": "hydroxychloroquine",
        "name": "Hydroxychloroquine",
        "pharmacologicClass": "disease-modifying-antirheumatic-drugs-dmards",
        "therapeuticClass": "antimalarials",
        "system": "antimicrobials",
        "mechanism": "Interfere with heme detoxification in Plasmodium parasites.",
        "features": "Less toxic than chloroquine, immunomodulatory. | Antimalarial with anti-inflammatory effects.",
        "clinicalChoice": "Rheumatoid arthritis, SLE, malaria prophylaxis.",
        "indications": [
            "Malaria",
            "Rheumatoid Arthritis",
            "SLE"
        ],
        "contraindications": [
            "Retinal Disease",
            "G6PD Deficiency"
        ],
        "sideEffects": [
            "Cinchonism",
            "Hemolysis in G6PD deficiency",
            "Retinal toxicity"
        ],
        "interactions": "QT-prolonging drugs increase arrhythmia risk.",
        "interactionDetails": "",
        "highYield": true,
        "pageType": "drug",
        "pharmacologicClasses": [
            "disease-modifying-antirheumatic-drugs-dmards",
            "quinoline-antimalarials"
        ]
    },
    {
        "id": "rifampin",
        "name": "Rifampin",
        "pharmacologicClass": "first-line-anti-tb-agents",
        "therapeuticClass": "anti-tuberculosis-drugs",
        "system": "antimicrobials",
        "mechanism": "Various mechanisms targeting mycobacterial metabolism and cell wall synthesis.",
        "features": "Potent CYP450 inducer, orange-red secretions.",
        "clinicalChoice": "First-line anti-TB therapy.",
        "indications": [
            "Tuberculosis",
            "Latent TB"
        ],
        "contraindications": [
            "Severe Liver Disease"
        ],
        "sideEffects": [
            "Hepatotoxicity",
            "Peripheral neuropathy",
            "Optic neuritis"
        ],
        "interactions": "Rifampin is major CYP450 inducer.",
        "interactionDetails": "",
        "pageType": "drug"
    },
    {
        "id": "isoniazid",
        "name": "Isoniazid",
        "pharmacologicClass": "first-line-anti-tb-agents",
        "therapeuticClass": "anti-tuberculosis-drugs",
        "system": "antimicrobials",
        "mechanism": "Various mechanisms targeting mycobacterial metabolism and cell wall synthesis.",
        "features": "Bactericidal against actively dividing mycobacteria.",
        "clinicalChoice": "First-line anti-TB, latent TB treatment.",
        "indications": [
            "Tuberculosis",
            "Latent TB"
        ],
        "contraindications": [
            "Severe Liver Disease",
            "Peripheral Neuropathy"
        ],
        "sideEffects": [
            "Hepatotoxicity",
            "Peripheral neuropathy",
            "Optic neuritis"
        ],
        "interactions": "Rifampin is major CYP450 inducer.",
        "interactionDetails": "",
        "pageType": "drug"
    },
    {
        "id": "ethambutol",
        "name": "Ethambutol",
        "pharmacologicClass": "first-line-anti-tb-agents",
        "therapeuticClass": "anti-tuberculosis-drugs",
        "system": "antimicrobials",
        "mechanism": "Various mechanisms targeting mycobacterial metabolism and cell wall synthesis.",
        "features": "Bacteriostatic, monitor for optic neuritis.",
        "clinicalChoice": "First-line anti-TB to prevent resistance.",
        "indications": [
            "Tuberculosis"
        ],
        "contraindications": [
            "Optic Neuritis",
            "Renal Impairment"
        ],
        "sideEffects": [
            "Hepatotoxicity",
            "Peripheral neuropathy",
            "Optic neuritis"
        ],
        "interactions": "Rifampin is major CYP450 inducer.",
        "interactionDetails": "",
        "pageType": "drug"
    },
    {
        "id": "pyrazinamide",
        "name": "Pyrazinamide",
        "pharmacologicClass": "first-line-anti-tb-agents",
        "therapeuticClass": "anti-tuberculosis-drugs",
        "system": "antimicrobials",
        "mechanism": "Various mechanisms targeting mycobacterial metabolism and cell wall synthesis.",
        "features": "Active in acidic environment, sterilizing activity.",
        "clinicalChoice": "Intensive phase anti-TB therapy.",
        "indications": [
            "Tuberculosis"
        ],
        "contraindications": [
            "Severe Liver Disease",
            "Gout"
        ],
        "sideEffects": [
            "Hepatotoxicity",
            "Peripheral neuropathy",
            "Optic neuritis"
        ],
        "interactions": "Rifampin is major CYP450 inducer.",
        "interactionDetails": "",
        "pageType": "drug"
    },
    {
        "id": "daptomycin",
        "name": "Daptomycin",
        "pharmacologicClass": "lipopeptides",
        "therapeuticClass": "antibacterials",
        "system": "antimicrobials",
        "mechanism": "Lipopeptide that inserts into bacterial cell membrane causing rapid depolarization and cell death.",
        "features": "Bactericidal against gram-positive organisms including MRSA and VRE, calcium-dependent.",
        "clinicalChoice": "Complicated skin/soft tissue infections, S. aureus bacteremia, right-sided endocarditis (MRSA/MSSA).",
        "indications": [
            "MRSA Bacteremia",
            "Right-Sided Endocarditis",
            "Complicated Skin/Soft Tissue Infections",
            "VRE Infections"
        ],
        "contraindications": [
            "Pneumonia (inactivated by pulmonary surfactant)"
        ],
        "sideEffects": [
            "CPK elevation (myopathy, rhabdomyolysis - monitor weekly)",
            "Eosinophilic pneumonia (rare)",
            "Peripheral neuropathy",
            "Injection site reactions"
        ],
        "interactions": "Statins (increased myopathy risk), warfarin (may increase INR).",
        "interactionDetails": "STATINS: MECHANISM - Both daptomycin and statins can cause myopathy and rhabdomyolysis. Daptomycin disrupts muscle cell membranes, statins impair muscle metabolism. CONSEQUENCE - Significantly increased risk of CPK elevation, myopathy, and rhabdomyolysis. Consider discontinuing statin during daptomycin therapy. Monitor CPK weekly. HMG-CoA REDUCTASE INHIBITORS: Similar mechanism - additive muscle toxicity.",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "linezolid",
        "name": "Linezolid",
        "pharmacologicClass": "oxazolidinones",
        "therapeuticClass": "antibacterials",
        "system": "antimicrobials",
        "mechanism": "Inhibits bacterial protein synthesis by binding to 23S ribosomal RNA of 50S subunit, preventing formation of 70S initiation complex.",
        "features": "Bacteriostatic, excellent oral bioavailability (100%), penetrates CSF, active against MRSA and VRE.",
        "clinicalChoice": "VRE infections, MRSA pneumonia (where vancomycin penetration poor), complicated skin infections.",
        "indications": [
            "VRE Infections",
            "MRSA Pneumonia",
            "Complicated Skin/Soft Tissue Infections",
            "MDR Gram-Positive Infections"
        ],
        "contraindications": [
            "Concurrent MAOIs (or within 2 weeks)",
            "Uncontrolled hypertension",
            "Pheochromocytoma",
            "Carcinoid syndrome"
        ],
        "sideEffects": [
            "Myelosuppression (thrombocytopenia, anemia - monitor CBC weekly)",
            "Peripheral neuropathy (prolonged use >28 days)",
            "Optic neuropathy (irreversible - prolonged use)",
            "Serotonin syndrome (weak MAO inhibitor)",
            "Lactic acidosis (inhibits mitochondrial protein synthesis)"
        ],
        "interactions": "Serotonergic drugs (SSRIs, SNRIs, tramadol - serotonin syndrome), tyramine-rich foods (hypertensive crisis).",
        "interactionDetails": "SEROTONERGIC DRUGS: MECHANISM - Linezolid is a weak, reversible MAO inhibitor. Combined with SSRIs, SNRIs, tramadol, or other serotonergic drugs increases serotonin levels. CONSEQUENCE - Serotonin syndrome (hyperthermia, agitation, hyperreflexia, tremor, confusion). Avoid concurrent serotonergic drugs. TYRAMINE-RICH FOODS: MECHANISM - MAO inhibition prevents tyramine breakdown, causing norepinephrine release. CONSEQUENCE - Hypertensive crisis. Avoid aged cheese, cured meats, red wine, fermented foods.",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "tigecycline",
        "name": "Tigecycline",
        "pharmacologicClass": "glycylcyclines",
        "therapeuticClass": "antibacterials",
        "system": "antimicrobials",
        "mechanism": "Glycylcycline (tetracycline derivative) that inhibits bacterial protein synthesis by binding 30S ribosomal subunit.",
        "features": "Broad-spectrum including MDR organisms (MRSA, VRE, ESBL, carbapenem-resistant), bypasses common tetracycline resistance.",
        "clinicalChoice": "Complicated intra-abdominal infections, complicated skin infections, polymicrobial MDR infections (reserve for resistant organisms).",
        "indications": [
            "Complicated Intra-Abdominal Infections",
            "Complicated Skin/Soft Tissue Infections",
            "Community-Acquired Pneumonia",
            "MDR Gram-Positive/Negative Infections"
        ],
        "contraindications": [
            "Pregnancy (Category D - tooth discoloration)",
            "Children <8 years"
        ],
        "sideEffects": [
            "Nausea/vomiting (very common, dose-limiting)",
            "Increased mortality (FDA black box - avoid unless no alternatives)",
            "Pancreatitis",
            "Hepatotoxicity",
            "Photosensitivity",
            "Tooth discoloration"
        ],
        "interactions": "Warfarin (decreased INR), oral contraceptives (decreased efficacy).",
        "interactionDetails": "WARFARIN: MECHANISM - Tigecycline may alter gut flora producing vitamin K, affecting warfarin metabolism. CONSEQUENCE - Decreased INR, reduced anticoagulation. Monitor INR closely. ORAL CONTRACEPTIVES: MECHANISM - Similar to tetracyclines, may reduce contraceptive efficacy through gut flora alteration or hepatic metabolism changes. CONSEQUENCE - Contraceptive failure. Use backup contraception.",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "colistin",
        "name": "Colistin (Polymyxin E)",
        "pharmacologicClass": "polymyxins",
        "therapeuticClass": "antibacterials",
        "system": "antimicrobials",
        "mechanism": "Cationic polypeptide that disrupts bacterial cell membrane via interaction with lipopolysaccharide, causing cell lysis.",
        "features": "Last-resort antibiotic for MDR gram-negative organisms (carbapenem-resistant), narrow therapeutic index.",
        "clinicalChoice": "Carbapenem-resistant Enterobacteriaceae (CRE), MDR Pseudomonas, MDR Acinetobacter - only when no alternatives.",
        "indications": [
            "Carbapenem-Resistant Enterobacteriaceae (CRE)",
            "MDR Pseudomonas aeruginosa",
            "MDR Acinetobacter baumannii",
            "Last-resort for MDR Gram-Negatives"
        ],
        "contraindications": [
            "Hypersensitivity to polymyxins"
        ],
        "sideEffects": [
            "Nephrotoxicity (dose-dependent, reversible - monitor Cr)",
            "Neurotoxicity (paresthesias, dizziness, ataxia, neuromuscular blockade)",
            "Bronchospasm (inhaled form)",
            "Apnea (high doses)",
            "Electrolyte disturbances"
        ],
        "interactions": "Nephrotoxic drugs (aminoglycosides, vancomycin - additive nephrotoxicity), neuromuscular blockers (prolonged paralysis).",
        "interactionDetails": "NEPHROTOXIC DRUGS: MECHANISM - Colistin causes dose-dependent nephrotoxicity. Additive with aminoglycosides, vancomycin, NSAIDs, contrast dye. CONSEQUENCE - Acute kidney injury. Avoid concurrent nephrotoxins when possible. Monitor renal function closely. NEUROMUSCULAR BLOCKERS: MECHANISM - Colistin can cause neuromuscular blockade by inhibiting acetylcholine release. Additive with succinylcholine, rocuronium, vecuronium. CONSEQUENCE - Prolonged paralysis, respiratory depression. Use with extreme caution in surgical/ICU settings.",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "fosfomycin",
        "name": "Fosfomycin",
        "pharmacologicClass": "cell-wall-synthesis-inhibitors",
        "therapeuticClass": "antibacterials",
        "system": "antimicrobials",
        "mechanism": "Inhibits MurA enzyme (UDP-N-acetylglucosamine enolpyruvyl transferase), blocking peptidoglycan synthesis at an earlier step than beta-lactams.",
        "features": "Single-dose oral treatment for uncomplicated UTI, achieves high urinary concentrations.",
        "clinicalChoice": "Uncomplicated cystitis (single-dose therapy), UTI caused by ESBL-producing organisms, patient convenience.",
        "indications": [
            "Uncomplicated Urinary Tract Infection (Cystitis)",
            "ESBL-Producing UTI"
        ],
        "contraindications": [
            "Pyelonephritis (inadequate tissue penetration)",
            "Complicated UTI"
        ],
        "sideEffects": [
            "Diarrhea",
            "Nausea",
            "Headache",
            "Vaginal candidiasis",
            "Well tolerated overall"
        ],
        "interactions": "Metoclopramide (decreases fosfomycin absorption).",
        "interactionDetails": "METOCLOPRAMIDE: MECHANISM - Metoclopramide increases GI motility, decreasing intestinal transit time and fosfomycin absorption. CONSEQUENCE - Reduced fosfomycin efficacy. Avoid concurrent use or give fosfomycin first, separate by several hours.",
        "highYield": true,
        "pageType": "drug"
    },
    {
        "id": "fidaxomicin",
        "name": "Fidaxomicin",
        "pharmacologicClass": "macrolides",
        "therapeuticClass": "antibacterials",
        "system": "antimicrobials",
        "mechanism": "Macrocyclic antibiotic that inhibits bacterial RNA polymerase, preventing transcription. Narrow spectrum, preserves normal gut flora better than vancomycin.",
        "features": "Narrow spectrum targeting C. difficile, minimal systemic absorption, lower recurrence rates than vancomycin.",
        "clinicalChoice": "C. difficile infection (first recurrence, or initial severe episode), preferred over vancomycin for recurrence prevention.",
        "indications": [
            "Clostridioides difficile Infection",
            "Recurrent C. difficile Infection"
        ],
        "contraindications": [
            "Hypersensitivity to fidaxomicin"
        ],
        "sideEffects": [
            "Nausea",
            "Vomiting",
            "Abdominal pain",
            "GI bleeding (rare)",
            "Anemia",
            "Well tolerated - similar to placebo"
        ],
        "interactions": "P-glycoprotein inhibitors (cyclosporine may increase fidaxomicin levels).",
        "interactionDetails": "P-GLYCOPROTEIN INHIBITORS: MECHANISM - Fidaxomicin is a P-gp substrate. Inhibitors (cyclosporine, verapamil) may increase systemic exposure. CONSEQUENCE - Unknown clinical significance due to minimal systemic absorption. Monitor for increased side effects if combined with strong P-gp inhibitors.",
        "highYield": true,
        "pageType": "drug"
    }

];export default drugs;
