...
const cvDiseases = [
    ...
    },
    {
        ...
        system: "cardiovascular",
        categories: ["cardiomyopathies"],
        symptoms: ["dyspnea", "fatigue", "orthopnea", "paroxysmal-nocturnal-dyspnea"],
        tags: ["heart-failure", "diastolic-dysfunction", "LVH", "HFpEF"],
        severity: "medium",
        acuity: "chronic",
        pageType: "disease",
        detail: {
            definition: "Cardiac structural and functional changes resulting from chronic systemic hypertension. Characterized by concentric left ventricular hypertrophy (LVH) → diastolic dysfunction → heart failure with preserved ejection fraction (HFpEF). Most common cause of HFpEF. Distinguished from genetic hypertrophic cardiomyopathy (HCM) by concentric (not asymmetric) hypertrophy and history of chronic HTN.",
            pathophysiology: "Chronic pressure overload → compensatory concentric LVH (sarcomeres added in parallel → wall thickening without chamber dilation). Initially adaptive (↓ wall stress per Laplace law). Progressive myocyte hypertrophy + interstitial fibrosis → ↓ ventricular compliance → diastolic dysfunction (impaired relaxation, ↑ filling pressures). Normal systolic function (EF preserved) but ↓ diastolic filling → ↓ cardiac output, pulmonary congestion → HFpEF. Coronary microvascular dysfunction → ischemia despite patent epicardial vessels. Atrial enlargement from chronic ↑ filling pressures → atrial fibrillation risk.",
            epidemiology: "Most common cause of LVH and HFpEF. Prevalence correlates with HTN prevalence (30-40% of adults). Increases with age. More common in African Americans (higher HTN prevalence, more severe LVH). 50% of heart failure patients have HFpEF, majority due to hypertensive heart disease.",
            riskFactors: ["Chronic uncontrolled hypertension (most important)", "Age >60", "African American race", "Obesity", "Diabetes", "Female sex (HFpEF more common in women)", "Chronic kidney disease", "Obstructive sleep apnea"],
            presentation: "Often asymptomatic early stage (compensated LVH). Progressive: dyspnea on exertion (DOE) → orthopnea, paroxysmal nocturnal dyspnea (PND) as diastolic dysfunction worsens. Fatigue, exercise intolerance. Lower extremity edema. Palpitations if atrial fibrillation develops. Angina possible (microvascular dysfunction, ↑ myocardial O2 demand from LVH) despite normal coronaries. History: longstanding poorly controlled HTN.",
            physicalExam: ["Elevated BP (ongoing HTN)", "Cardiac: sustained PMI (LVH), LV heave (forceful apical impulse)", "S4 gallop: stiff/noncompliant LV (atrial kick against stiff ventricle). Key finding in diastolic dysfunction.", "If decompensated HFpEF: JVD, bibasilar crackles, peripheral edema, S3 gallop (volume overload)", "Fundoscopy: hypertensive retinopathy (AV nicking, copper wiring)"],
            diagnosticCriteria: "Clinical diagnosis: chronic HTN + evidence of LVH (ECG or echo) ± diastolic dysfunction ± HFpEF symptoms. Distinguished from HCM by concentric (not asymmetric) pattern and HTN history.",
            labs: ["BNP/NT-proBNP: elevated if HFpEF (↑ wall stress). Normal if compensated.", "BMP: assess renal function (HTN nephropathy common), electrolytes", "Lipid panel, HbA1c: cardiovascular risk factors", "Troponin: if acute decompensation or angina (rule out ACS)"],
            imaging: ["ECG: LVH criteria (Sokolow-Lyon: S in V1 + R in V5/V6 >35mm; Cornell: S in V3 + R in aVL >28mm men, >20mm women). Strain pattern (downsloping ST depression, T wave inversion in lateral leads I, aVL, V5-V6). Left atrial enlargement (wide P wave in lead II, biphasic P in V1).", "Echocardiography (GOLD STANDARD): Concentric LVH (increased wall thickness, normal or small LV cavity, increased relative wall thickness RWT >0.42). Normal/preserved LVEF (≥50%). Diastolic dysfunction: E/A reversal (impaired relaxation), ↑ E/e' ratio (↑ filling pressures), left atrial enlargement. Distinguish from HCM: HCM has asymmetric septal hypertrophy, SAM, LVOT obstruction.", "Cardiac MRI: if echo inadequate. Assess fibrosis (late gadolinium enhancement)."],
            differentialDiagnosis: ["Hypertrophic cardiomyopathy (HCM): asymmetric septal hypertrophy (septum ≥1.5x posterior wall), LVOT obstruction, SAM, genetic/familial. Hypertensive heart disease: concentric LVH, HTN history, no LVOT obstruction.", "HFrEF (systolic heart failure): reduced EF <40%, dilated LV. Hypertensive heart disease: preserved EF ≥50%, normal/small LV, concentric LVH.", "Restrictive cardiomyopathy: infiltrative (amyloid, sarcoid), biatrial enlargement, very stiff ventricles, low voltage ECG (amyloid). Hypertensive heart disease: LVH on ECG, HTN history.", "Athlete's heart: physiologic LVH from training. Normal diastolic function, wall thickness <13mm, ↑ LV cavity size. Regresses with deconditioning. Hypertensive heart disease: diastolic dysfunction, wall thickness often >13mm, small cavity, HTN.", "Aortic stenosis: LVH from pressure overload, but valvular stenosis present (systolic murmur, small pulse pressure, ↓ valve area on echo).", "Obesity/deconditioning: DOE, but no LVH, normal echo."],
            management: {
                acute: ["If acute decompensated HFpEF: LMNOP - Lasix (IV diuresis), Morphine (if severe distress), Nitrates (↓ preload), Oxygen, Position (upright). Goal: ↓ pulmonary congestion.", "IV diuretics: furosemide 40-80mg IV. Monitor urine output, daily weights, avoid overdiuresis (preload-dependent).", "Treat hypertensive emergency if present: IV nicardipine, labetalol. Gradual BP reduction.", "Rule out ACS (troponin, ECG) and other precipitants (afib, anemia, infection, nonadherence)"],
                chronic: ["BP control MOST IMPORTANT: target <130/80. ACE-I or ARB first-line (↓ afterload, regression of LVH, ↓ fibrosis). Amlodipine, chlorthalidone also effective.", "Diuretics: for volume management if symptoms. Loop (furosemide) or thiazide. Avoid overdiuresis (↓ preload → ↓ CO in HFpEF).", "SGLT2 inhibitors (empagliflozin, dapagliflozin): REDUCE HFpEF HOSPITALIZATIONS (EMPEROR-Preserved, DELIVER trials). Emerging first-line for HFpEF.", "Treat comorbidities: diabetes (glycemic control), obesity (weight loss improves symptoms), OSA (CPAP), afib (rate control, anticoagulation)", "Avoid: dehydration (preload-dependent), aggressive diuresis, negative inotropes if possible (β-blockers may worsen in some, use cautiously)", "Aldosterone antagonist (spironolactone): modest benefit in TOPCAT trial (↓ hospitalizations). Consider if refractory.", "Lifestyle: sodium restriction (<2g/day), weight loss if obese, exercise training (improves functional capacity)", "Monitor: serial echo to assess LVH regression with treatment, BNP if HFpEF"]
            },
            complications: ["Heart failure with preserved EF (HFpEF): most common consequence", "Atrial fibrillation: 30-40% (LA enlargement from chronic ↑ pressures)", "Coronary microvascular dysfunction → angina, ischemia despite normal epicardial coronaries", "Sudden cardiac death: arrhythmic (VT/VF from fibrosis, LVH)", "Stroke: from afib, HTN", "Chronic kidney disease: concurrent HTN nephropathy", "Acute decompensated heart failure: hospitalizations"],
            prognosis: "Variable. With BP control: LVH may regress partially, diastolic function may stabilize/improve. Without treatment: progressive diastolic dysfunction → HFpEF → frequent hospitalizations, poor quality of life. HFpEF mortality 5-year ~50% (similar to HFrEF). Sudden death risk if severe LVH.",
            clinicalPearls: ["Hypertensive heart disease = chronic HTN → concentric LVH → diastolic dysfunction → HFpEF. Most common cause of HFpEF.", "Distinguished from HCM: CONCENTRIC (not asymmetric) LVH, HTN history, no LVOT obstruction, no family history.", "S4 gallop = hallmark of diastolic dysfunction (stiff LV). Hear in hypertensive heart disease, before HFpEF develops.", "Echo findings: concentric LVH (↑ wall thickness, normal/small cavity, RWT >0.42), preserved EF ≥50%, diastolic dysfunction (E/A reversal, ↑ E/e'), LA enlargement.", "ECG: LVH with strain pattern (lateral ST depression, T wave inversion in I, aVL, V5-V6).", "BP control MOST IMPORTANT: ACE-I/ARB first-line (↓ afterload, LVH regression). Target <130/80.", "SGLT2 inhibitors (empagliflozin, dapagliflozin) REDUCE HFpEF HOSPITALIZATIONS. Emerging standard for HFpEF (including hypertensive heart disease).", "HFpEF management: diuretics for symptoms (but avoid overdiuresis - preload-dependent), SGLT2i, BP control, treat comorbidities. Unlike HFrEF: no proven mortality benefit from ACE-I, β-blockers.", "Concentric LVH = parallel sarcomere addition → wall thickening without dilation. Asymmetric (HCM) = sarcomere mutations → disorganized hypertrophy.", "Atrial fibrillation common (30-40%) from chronic LA stretch. Anticoagulate per CHA2DS2-VASc.", "Weight loss, exercise training, sodium restriction improve symptoms in HFpEF.", "If angina with normal coronaries: suspect microvascular dysfunction from LVH. Treat HTN, consider ranolazine."]
        }
    }
    ,
    {
    ...
    { id: "aortic-stenosis", name: "Aortic Stenosis", system: "cardiovascular", categories: ["valvular-disease"], severity: "high", acuity: "chronic", tags: ["crescendo-decrescendo", "syncope", "angina"], pageType: "disease", detail: { definition: "Narrowing of aortic valve. Causes: senile calcific (most common >65yo), bicuspid aortic valve (<65yo), rheumatic heart disease. Triad: angina, syncope, dyspnea (exertional). Crescendo-decrescendo systolic murmur at RUSB radiating to carotids, paradoxically split S2 (severe), delayed/weak carotid upstrokes (pulsus parvus et tardus). Echo: valve area <1cm² (severe), mean gradient >40mmHg. ECG: LVH. Treat: valve replacement (surgical AVR or TAVR) if symptomatic or severe. Poor prognosis once symptomatic (50% mortality at 2 years if untreated).", overview: "[Content coming soon]" } },
    { id: "aortic-regurgitation", name: "Aortic Regurgitation", system: "cardiovascular", categories: ["valvular-disease"], severity: "high", acuity: "chronic", tags: ["diastolic-murmur", "bounding-pulse", "widened-pulse-pressure"], pageType: "disease", detail: { definition: "Incompetent aortic valve. Causes: aortic root dilation (HTN, Marfan, syphilitic aortitis), endocarditis, rheumatic heart disease, bicuspid valve, aortic dissection (acute). Chronic: exertional dyspnea, angina. Acute: sudden dyspnea, pulmonary edema, shock. Early diastolic decrescendo murmur at LUSB (leaning forward), Austin Flint murmur (rumble at apex - regurgitant jet hits mitral valve). Bounding pulses, widened pulse pressure. Water-hammer pulse (Corrigan), head bobbing (de Musset), pistol shot femoral pulse (Traube), Quincke (nail bed capillary pulsations). Echo: LV dilation. Treat: AVR if symptomatic, LV dysfunction, or severe AR. Acute AR: emergency surgery.", overview: "[Content coming soon]" } },
    { id: "mitral-stenosis", name: "Mitral Stenosis", system: "cardiovascular", categories: ["valvular-disease"], severity: "high", acuity: "chronic", tags: ["opening-snap", "AFib", "rheumatic"], pageType: "disease", detail: { definition: "Narrowing of mitral valve. Cause: rheumatic heart disease (almost always). Dyspnea on exertion, orthopnea, hemoptysis. AFib common (dilated LA). Diastolic rumble at apex with opening snap, loud S1, accentuated by left lateral position. Malar flush (mitral facies). Echo: valve area <1.5cm² (severe), LA enlargement. CXR: LA enlargement (straightened left heart border, double density, splaying of carina), Kerley B lines (pulmonary edema). Complications: AFib, LA thrombus/emboli, pulmonary HTN. Treat: diuretics, rate control if AFib, anticoagulation if AFib, balloon valvotomy or surgical repair/replacement if severe.", overview: "[Content coming soon]" } },
    { id: "mitral-regurgitation", name: "Mitral Regurgitation", system: "cardiovascular", categories: ["valvular-disease"], severity: "high", acuity: "chronic", tags: ["holosystolic-murmur", "AFib", "S3"], pageType: "disease", detail: { definition: "Incompetent mitral valve. Causes: MVP (most common), ischemic (papillary muscle dysfunction/rupture), rheumatic, endocarditis, dilated cardiomyopathy. Chronic: exertional dyspnea, fatigue. Acute (papillary rupture post-MI): sudden pulmonary edema, shock. Holosystolic murmur at apex radiating to axilla, S3, displaced PMI (LV dilation). AFib common. Echo: regurgitant jet, LA/LV dilation. Treat: vasodilators (ACE-I, hydralazine), diuretics. Surgery (repair > replacement) if symptomatic, LV dysfunction, or severe MR. Acute MR: emergency surgery.", overview: "[Content coming soon]" } },
    { id: "atrial-fibrillation", name: "Atrial Fibrillation", system: "cardiovascular", categories: ["syncope-palpitations"], severity: "high", acuity: "acute", tags: ["irregularly-irregular", "stroke-risk", "CHA2DS2-VASc"], pageType: "disease", detail: { definition: "Chaotic atrial activity. Causes: HTN, CHF, valvular (mitral), CAD, thyrotoxicosis, alcohol (holiday heart), PE, post-cardiac surgery, lone (no structural). Palpitations, dyspnea, chest discomfort, stroke/TIA. Irregularly irregular rhythm, no P waves, variable RR intervals. Complications: stroke (5x risk), heart failure, tachycardia-induced cardiomyopathy. Treat: Rate control (beta-blocker, CCB) vs rhythm control (cardioversion + antiarrhythmic). Anticoagulation: CHA2DS2-VASc score ≥2 men/≥3 women → DOAC or warfarin. Cardioversion: if <48hrs or TEE no thrombus. Ablation if refractory.", overview: "[Content coming soon]" } },
    { id: "ventricular-tachycardia", name: "Ventricular Tachycardia", system: "cardiovascular", categories: ["syncope-palpitations"], severity: "high", acuity: "acute", tags: ["wide-complex", "emergency", "ICD"], pageType: "disease", detail: { definition: "≥3 consecutive ventricular beats at rate >100. Monomorphic (uniform QRS) vs polymorphic (variable QRS - torsades). Causes: CAD/MI (most common), cardiomyopathy, channelopathies (long QT, Brugada), medications (QT prolongation). Pulseless VT: cardiac arrest, treat as VF. Stable VT: palpitations, dyspnea, chest pain, syncope. Wide complex tachycardia (>120ms), AV dissociation (pathognomonic). Treat: Pulseless → defibrillation. Stable monomorphic → amiodarone or procainamide, cardioversion if unstable. Polymorphic (torsades) → IV magnesium, correct QT. Long-term: ICD (secondary prevention if sustained VT), antiarrhythmics, ablation. Treat underlying (revascularization if ischemic).", overview: "[Content coming soon]" } },
    { id: "heart-block", name: "Heart Blocks (AV Blocks)", system: "cardiovascular", categories: ["syncope-palpitations"], severity: "high", acuity: "acute", tags: ["bradycardia", "AV-dissociation", "pacemaker"], pageType: "disease", detail: { definition: "Impaired AV conduction. 1st degree: prolonged PR >200ms (benign). 2nd degree Mobitz I (Wenckebach): progressive PR prolongation until dropped QRS (benign, AV node). 2nd degree Mobitz II: constant PR with intermittent dropped QRS (dangerous, infranodal - risk progression to 3rd degree). 3rd degree (complete): no AV conduction, AV dissociation, ventricular escape rhythm. Causes: MI (inferior → AV node, anterior → infranodal), meds (beta-blockers, CCB, digoxin), fibrosis, Lyme, endocarditis. Symptomatic: syncope, dizziness, fatigue, dyspnea. Treat: 1st degree - none. Mobitz I - observe. Mobitz II or 3rd degree - pacemaker. Acute MI with heart block: atropine (if AV node), temporary pacing, permanent pacemaker if persistent.", overview: "[Content coming soon]" } },
    { id: "aortic-dissection-detail", name: "Aortic Dissection", system: "cardiovascular", categories: ["chest-pain", "vascular-disease"], severity: "high", acuity: "acute", tags: ["tearing-pain", "emergency", "widened-mediastinum"], pageType: "disease", detail: { definition: "Tear in aortic intima → blood enters media, creates false lumen. Stanford A (ascending aorta, 60% - surgical emergency) vs B (descending, distal to left subclavian, 40% - medical management). Causes: HTN (most important), connective tissue disease (Marfan, Ehlers-Danlos), bicuspid aortic valve, cocaine, trauma. Sudden tearing/ripping chest pain radiating to back, may migrate as dissection extends. BP differential between arms (>20mmHg), pulse deficit, AR murmur (type A), neurologic deficits (stroke, paraplegia), mesenteric ischemia. CXR: widened mediastinum (>8cm), pleural effusion. CT angiography or TEE: diagnostic. Treat: Type A → emergency surgery. Type B → medical (beta-blocker to HR <60, SBP <120), surgery if complicated (rupture, malperfusion, progression). Mortality: type A 1-2%/hr if untreated.", overview: "[Content coming soon]" } }
];
